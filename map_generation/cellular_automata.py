import random
from map_generation.map_generator import MapGernerator
from sprites.floor import Floor
from sprites.wall import Wall
from sprites.boundary_wall import Boundary_Wall


class Cellular_Automata(MapGernerator):
    def __init__(self, actual_map_height, actual_map_width, wall_density, wall_count_variable, iterations, camera_group, game):
        super().__init__(actual_map_height, actual_map_width) #inherit from MapGenerator superclass
        self.game = game 
        self.block_size = self.game.block_size  
        self.wall_density = wall_density 
        self.noise_grid = []
        self.camera_group = camera_group
        self.wall_count_variable = wall_count_variable #mimimum number of adjacent walls needed for a node to be set to a wall
        self.iterations = iterations
        self.final_map = [] #array for final map

    def generate_noise_grid(self):
        #create a 2-dimensional array of 1s and 0s randomly distributed
        for x in range (0, self.actual_map_width):
            new_array = []
            for y in range (0, self.actual_map_height):
                decision = random.randint(1,100)
                if decision <= self.wall_density:
                    new_array.append(0) #wall
                else:
                    new_array.append(1) #floor
            self.noise_grid.append(new_array)
        return(self.noise_grid)
    
    def fill_map(self, map):
        #method to instantiate wall and floor classes using the inputted map as a template
        for x in range (0, self.actual_map_width):
            for y in range (0, self.actual_map_height):
                if map[x][y] == 0: 
                    wall = Wall(x*self.block_size, y*self.block_size, self.camera_group, self.game) #instantiate wall class
                
                elif map[x][y] == 1:
                    floor = Floor(x*self.block_size, y*self.block_size, self.camera_group, self.game) #instantiate floor class

                elif map[x][y] == 2: 
                    boundary_wall = Boundary_Wall(x*self.block_size, y*self.block_size, self.camera_group, self.game) #instantiate wall class
                   
    def cellular_automata(self, noise_grid, iterations):
        for count in range (0, iterations):
            #iterate the algorithm for the number of iterations specified
            new_grid = noise_grid.copy() #make a copy of the noise grid to prevent overwridding the original
            for a in range (0,(self.actual_map_width)):
                for b in range (0,(self.actual_map_height)):
                    wall_count = 0 #reset wall_count
                    for x in range (a-1, a+2):
                        for y in range(b-1, b+2):
                            #examine all eight nodes around the examined node
                            if x != a or y !=b:
                                #check to ensure the examined node is not considered
                                if 0 <= x < self.actual_map_width and 0 <= y < self.actual_map_height:
                                    #check if the neiwghbour node is in map remits
                                    if new_grid[x][y] == 0:
                                        wall_count = wall_count + 1 #increment wall count if neighbour node is a wall
                                else:
                                    wall_count = wall_count + 1 #increment wall count by one if not in map remits
                    if wall_count > self.wall_count_variable:
                        noise_grid[a][b] = 0 #overwrite original noise_grid node to a wall if it has enough neighbouring walls
                    else:
                        noise_grid[a][b] = 1 #otherwise, set it to be a floor
        return(noise_grid)

    def fill_edges(self, map):
        #method to fill the outer perimeter of the map with walls
        for x in range(0, self.actual_map_width):
            for y in range(0, self.actual_map_height):
                if x == 0 or x == self.actual_map_width-1:
                    map[x][y] = 2
                if y == 0 or y == self.actual_map_height-1:
                    map[x][y] = 2
        return(map)
    
    def check_map_validity(self):
        #method to ensure map parameters are valid
        check = True
        if self.actual_map_height < 0 or self.actual_map_width < 0:
            #if map has negative dimensions
            check = False
            print("map cannot have negative dimensions")
        if self.wall_density < 0 or self.wall_density > 100:
            #if wall density not between 0 and 100
            check = False
            print("the wall density for map generation is not in the expected range") 
        if self.iterations < 0:
            #if iterations are negative
            check = False
            print("iterations must be a positive value")
        if self.wall_count_variable < 0:
            #if wall count variable is negative
            check = False
            print("wall_count_variable must be a positive value")
        return(check)
    
    def update(self):
        if self.check_map_validity() == True:
            #if map parameters are valid, call all methods to create a map
            noise_grid = self.generate_noise_grid()
            cellular_automata_map = self.cellular_automata(noise_grid, self.iterations)
            self.final_map = self.fill_edges(cellular_automata_map)
            self.fill_map(self.final_map)
            
